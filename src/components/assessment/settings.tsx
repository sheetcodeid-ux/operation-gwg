...
import { Loader2, Search, ShieldCheck, UserCog, Users, X } from "lucide-react";
...
export function AssessmentSettings({
  ...
  const roleOf = (id: string): RoleChoice => roster[id]?.role ?? "none";

  const filtered = React.useMemo(() => {
    const s = q.trim().toLowerCase();
    if (!s) return accounts;
    return accounts.filter(
      (a) => a.name.toLowerCase().includes(s) || a.email.toLowerCase().includes(s) || (a.department ?? "").toLowerCase().includes(s) || (a.jabatan ?? "").toLowerCase().includes(s),
    );
  }, [accounts, q]);
  ...
  const registered = accounts.filter((a) => roleOf(a.id) !== "none").length;
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="glass flex flex-wrap items-center gap-x-5 gap-y-2 rounded-2xl border border-border p-4">
        ...
        <Stat label="Terdaftar" value={registered} />
      ...
      <div className="glass rounded-2xl border border-border p-4 sm:p-5">
        <div className="mb-1 flex items-center gap-2">
          ...
          <p className="text-sm font-semibold text-foreground">Peran Akun di Assessment</p>
        </div>
        <p className="mb-3 text-[11px] text-muted-foreground">
          Data akun diambil dari User Management. Tetapkan peran tiap akun — peran menentukan tampilan yang mereka lihat. Akun <span className="font-medium">Belum terdaftar</span> tidak bisa membuka Assessment.
        </p>
        <div className="relative mb-3">
          <Search className="pointer-events-none absolute left-3 top-1/2 size-4 -translate-y-1/2 text-muted-foreground" />
          <Input value={q} onChange={(e) => setQ(e.target.value)} placeholder="Cari nama, email, divisi…" className="pl-9" />
        </div>

        <div className="space-y-2">
          {filtered.map((a) => {
            const role = roleOf(a.id);
            const isParticipant = role === "karyawan";
            const cur = assign[a.id] ?? { atasanUserId: null, peerUserIds: [] };
            const peerOptions = accounts
              .filter((x) => x.id !== a.id && !cur.peerUserIds.includes(x.id))
              .map((x) => ({ value: x.id, label: x.name, hint: x.department ?? undefined }));
            return (
              <div key={a.id} className="rounded-xl border border-border bg-muted/20 p-3">
                <div className="flex flex-wrap items-center gap-2">
                  <div className="min-w-0 flex-1">
                    ...
                    <p className="truncate text-[11px] text-muted-foreground">{[a.department, a.jabatan].filter(Boolean).join(" · ") || a.email}</p>
                  </div>
                  <div className="w-full sm:w-52">
                    <Combobox
                      ...
                      searchable={false}
                      value={role}
                      onChange={(v) => changeRole(a.id, v as RoleChoice)}
                      options={ROLE_OPTS.map((r) => ({ value: r.value, label: r.label }))}
                    />
                  </div>
                </div>
                ...
                {role === "head" && (
                  <div className="mt-2 flex flex-wrap items-center gap-2 border-t border-border/60 pt-2">
                    <span className="text-[11px] font-medium text-muted-foreground">Menilai divisi</span>
                    <div className="w-full sm:w-64">
                      <Combobox
                        portal
                        matchTriggerWidth
                        value={roster[a.id]?.scopeDepartmentId ?? ""}
                        onChange={(v) => changeScope(a.id, v)}
                        options={[{ value: "", label: "Divisinya sendiri (default)" }, ...departments]}
                ...
                {/* Participant → Atasan + Rekan Sejawat */}
                {isParticipant && (
                  <div className="mt-2 space-y-2 border-t border-border/60 pt-2">
                    <div className="flex flex-wrap items-center gap-2">
                      <span className="w-24 shrink-0 text-[11px] font-medium text-muted-foreground">Atasan (P1)</span>
                      <div className="w-full sm:w-64">
                        <Combobox
                          portal
                          matchTriggerWidth
                          value={cur.atasanUserId ?? ""}
                          onChange={(v) => setAtasan(a.id, v || null)}
                          options={[{ value: "", label: "— pilih atasan —" }, ...headOptions]}
                    ...
                    <div className="flex flex-wrap items-start gap-2">
                      <span className="w-24 shrink-0 pt-1.5 text-[11px] font-medium text-muted-foreground">Rekan Sejawat (P3)</span>
                      <div className="min-w-0 flex-1">
                        ...
                        {cur.peerUserIds.length < 5 ? (
                          <div className="mt-1.5 w-full sm:w-64">
                            <Combobox
                              ...
                              matchTriggerWidth
                              value=""
                              onChange={(v) => v && addPeer(a.id, v)}
                              options={peerOptions}
                              searchPlaceholder="Cari rekan…"
                              placeholder={`Tambah rekan sejawat (${cur.peerUserIds.length}/5)…`}
                            />
                          </div>
                        ) : (
          ...
          {filtered.length === 0 && <p className="py-6 text-center text-sm text-muted-foreground">Tidak ada akun cocok.</p>}
        </div>
      </div>
