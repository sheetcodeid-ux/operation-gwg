...
export function AssessmentQueue({ mode = "penilaian" }: { mode?: "penilaian" | "interview" }) {
  ...
  React.useEffect(() => {
    let live = true;
    getMyAssessmentTargets(mode).then((t) => { if (live) setTargets(t); });
    return () => { live = false; };
  }, [a.candidate.employeeId, mode]);
