...
export function InterviewTab() {
  ...
  // Interview yang sudah dikirim tetap bisa diubah — tidak ada penguncian.
  const myRow = a.session?.evaluations.find((x) => x.evaluatorKey === myKey);
  const mySubmitted = !!myKey && !!myRow?.submitted && !!myRow?.ivVote;
    ...
    <div className="space-y-4">
      ...
      {mySubmitted && (
        <Banner tone="success" icon={<CheckCircle2 className="size-4" />}>
          Interview Anda sudah <strong>terkirim</strong>. Masih bisa diubah — perbaiki lalu kirim ulang.
        </Banner>
      )}
...
function MyInterviewSave({ dimsDone }: { dimsDone: boolean }) {
  ...
  const onSave = async () => {
    ...
    setMsg(res.ok ? (saved ? "Interview Anda diperbarui." : "Interview Anda tersimpan.") : res.error ?? "Gagal menyimpan.");
  };
    ...
    <div className="space-y-2">
      {saved && (
        <div className="rounded-xl border border-brand-500/40 bg-brand-500/5 p-3 text-center text-sm font-medium text-brand-600 dark:text-brand-400">
          <CheckCircle2 className="mr-1.5 inline size-4" /> Interview Anda sudah terkirim — masih bisa diubah.
        </div>
      )}
      <Button className="h-12 w-full text-base" onClick={onSave} disabled={!ready || a.sessionBusy}>
        ...
        {saved ? "Perbarui Interview" : "Kirim Interview"}
      </Button>
      {saved && (
