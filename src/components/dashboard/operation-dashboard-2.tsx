 ...
 *
 * Layout follows the user's Figma but re-themed to the app (violet brand / Aniq UI,
 * NOT the mockup's red). Cards are drag-reorderable (per column). Data here is
 * deterministic PLACEHOLDER — marked `TODO(api)` where the ERP/DB will feed it in
 * later phases (Beban Operasional & Pembelian tables, gwgmanage reports API).
 */
...
/* ---------- theme palette (violet brand, no red) ---------- */
const C = { brand: "#8b5cf6", brand2: "#a78bfa", brand3: "#c4b5fd", emerald: "#22c55e", amber: "#f59e0b", cyan: "#06b6d4", slate: "#94a3b8", rose: "#f43f5e" };
...
const rpShort = (n: number) => {
  const v = Math.abs(n);
  const s = v >= 1e9 ? (n / 1e9).toFixed(1).replace(/\.0$/, "") + "M" : v >= 1e6 ? (n / 1e6).toFixed(0) + "jt" : v >= 1e3 ? (n / 1e3).toFixed(0) + "rb" : String(n);
  return "Rp" + s;
};

/* ---------- deterministic placeholder generators ---------- */
const rand = (seed: number) => { let s = seed; return () => ((s = (s * 1103515245 + 12345) & 0x7fffffff) / 0x7fffffff); };
const BRANDS_TABS = ["Semua", "Kalimantan", "Jawa", "Bali"];
const EXPENSE_CATS = [
    ...
    <div className="space-y-4">
      ...
      <div className="flex flex-wrap items-center gap-2">
        <div className="flex items-center gap-2 rounded-xl border border-border bg-card px-3 py-1.5 text-xs">
          ...
          <Select value={period} onChange={setPeriod} options={[{ v: "hari", l: "Per Hari" }, { v: "minggu", l: "Per Minggu" }, { v: "bulan", l: "Per Bulan 2026.03" }]} />
        </div>
        <div className="ml-auto flex flex-wrap items-center gap-2">
          <Select value={ca} onChange={setCa} options={[{ v: "all", l: "Semua CA" }, { v: "owner", l: "Owner" }, { v: "spv", l: "SPV" }]} className="rounded-xl border border-border bg-card px-3 py-1.5 text-xs" />
          <Select value={cabang} onChange={setCabang} options={[{ v: "all", l: "Semua Cabang" }, { v: "ccay", l: "Cattu A. Yani" }]} className="rounded-xl border border-border bg-card px-3 py-1.5 text-xs" />
          <button type="button" className="inline-flex items-center gap-1.5 rounded-xl bg-primary px-3.5 py-1.5 text-xs font-medium text-primary-foreground hover:opacity-90">
            <Eye className="size-3.5" /> Petinjau
          </button>
          <button type="button" className="inline-flex items-center gap-1.5 rounded-xl border border-border bg-card px-3.5 py-1.5 text-xs font-medium text-foreground hover:bg-muted">
      ...
      <div className="grid gap-4 lg:grid-cols-12">
        <div className="min-w-0 lg:col-span-3">
          <DraggableGrid items={left} storageKey="opsdash2-left" />
        </div>
        <div className="min-w-0 lg:col-span-6">
          <DraggableGrid items={mid} storageKey="opsdash2-mid" />
        </div>
        <div className="min-w-0 lg:col-span-3">
          <DraggableGrid items={right} storageKey="opsdash2-right" />
        </div>
      </div>
    </div>
  );
...
/* ---------- shared bits ---------- */
function Select({ value, onChange, options, className }: { value: string; onChange: (v: string) => void; options: { v: string; l: string }[]; className?: string }) {
  return (
    <select value={value} onChange={(e) => onChange(e.target.value)} className={cn("cursor-pointer bg-transparent font-medium text-foreground outline-none", className)}>
      {options.map((o) => (
        <option key={o.v} value={o.v} className="bg-popover text-foreground">{o.l}</option>
      ))}
    </select>
  );
}
...
function Delta({ v, positiveIsGood = true }: { v: number; positiveIsGood?: boolean }) {
  ...
  return (
    <span className={cn("inline-flex items-center gap-0.5 rounded-full px-1.5 py-0.5 text-[10.5px] font-semibold tabular-nums", good ? "bg-emerald-500/15 text-emerald-600 dark:text-emerald-400" : "bg-red-500/15 text-red-600 dark:text-red-400")}>
      {v >= 0 ? <ArrowUpRight className="size-3" /> : <ArrowDownRight className="size-3" />}
      {Math.abs(v)}%
    </span>
  );
}
function Bar2({ value, tone = "brand" }: { value: number; tone?: "brand" | "emerald" | "amber" }) {
  const bg = tone === "emerald" ? "bg-emerald-500" : tone === "amber" ? "bg-amber-500" : "bg-primary";
  return (
...
function TargetGauge() {
  const pct = 95.38, R = 52, circ = Math.PI * R; // semicircle
...
function ProgressCard({ title, pct, actual, target }: { title: string; pct: number; actual: number; target: number }) {
  return (
    <Panel>
      ...
      <div className="mt-2"><Bar2 value={pct} /></div>
      ...
      <div className="mt-3 space-y-3">
        {weeks.map((w) => (
          <div key={w.w}>
            ...
            <div className="mt-1"><Bar2 value={w.p * 4} /></div>
...
function ProdukCard() {
  const [tab, setTab] = React.useState<"jumlah" | "total">("total");
  ...
  return (
    <Panel>
      <Title right={<span className="text-[11px] text-muted-foreground">Kategori ▾</span>}>Produk</Title>
      <div className="mt-2 flex gap-1 border-b border-border text-[12px]">
        {(["jumlah", "total"] as const).map((t) => (
          <button key={t} onClick={() => setTab(t)} className={cn("px-2 pb-1.5 font-medium capitalize", tab === t ? "border-b-2 border-primary text-foreground" : "text-muted-foreground")}>{t}</button>
        ))}
      </div>
...
function KpiCluster() {
  ...
  return (
    <div className="grid grid-cols-2 gap-4">
      {items.map((it) => (
        <Panel key={it.label} className="p-4">
          <div className="flex items-center justify-between gap-2">
            <span className="grid size-8 place-items-center rounded-lg bg-primary/12"><it.icon className="size-4 text-primary" /></span>
            <Delta v={it.d} />
          </div>
          <p className="mt-3 text-sm text-muted-foreground">{it.label}</p>
          <p className="text-lg font-bold tabular-nums text-foreground">{rp(it.value)}</p>
...
function PenjualanChart() {
  ...
  return (
    <Panel>
      <Title right={<Select value={mode} onChange={setMode} options={[{ v: "harian", l: "Harian" }, { v: "mingguan", l: "Mingguan" }]} className="rounded-lg border border-border bg-card px-2 py-1 text-[11px]" />}>Penjualan</Title>
      <div className="mt-1 flex items-center gap-2">
        <p className="text-lg font-bold tabular-nums text-foreground">{rp(100_000_000)}</p>
        <Delta v={1.78} />
      </div>
...
function BebanChart() {
  ...
  return (
    <Panel>
      <Title right={<Select value={mode} onChange={setMode} options={[{ v: "persentase", l: "Persentase" }, { v: "nominal", l: "Nominal" }]} className="rounded-lg border border-border bg-card px-2 py-1 text-[11px]" />}>Beban Operasional</Title>
      <div className="mt-1 flex items-center gap-2">
        <p className="text-lg font-bold tabular-nums text-foreground">{rp(100_000_000)}</p>
        <Delta v={1.78} />
      </div>
      <div className="mt-2 flex flex-wrap gap-x-3 gap-y-1">
        {EXPENSE_CATS.map((c) => (
          <span key={c.key} className="flex items-center gap-1 text-[10.5px] text-muted-foreground"><span className="size-2 rounded-full" style={{ background: c.color }} /> {c.key}</span>
        ))}
      </div>
        ...
        <ResponsiveContainer width="100%" height="100%">
          <BarChart data={data} margin={{ top: 6, right: 4, left: -20, bottom: 0 }}>
            ...
            {EXPENSE_CATS.map((c, i) => (
              <Bar key={c.key} dataKey={c.key} stackId="e" fill={c.color} radius={i === EXPENSE_CATS.length - 1 ? [3, 3, 0, 0] : undefined} />
            ))}
          </BarChart>
        </ResponsiveContainer>
      </div>
...
function PerformaCabang() {
  ...
  return (
    <Panel>
      <Title right={<button className="grid size-7 place-items-center rounded-lg border border-border text-muted-foreground hover:bg-muted"><SlidersHorizontal className="size-3.5" /></button>}>Performa Cabang</Title>
      <div className="mt-3 overflow-x-auto">
        <table className="w-full text-[12px]">
          <thead>
            <tr className="text-left text-muted-foreground">
              <th className="px-2 py-1.5 font-medium">#</th><th className="px-2 py-1.5 font-medium">Nama Cabang</th><th className="px-2 py-1.5 font-medium">Mei 2026</th><th className="px-2 py-1.5 font-medium">Jun 2026</th><th className="px-2 py-1.5 text-right font-medium">Persentase</th>
            </tr>
          </thead>
          <tbody>
            {rows.map((row) => (
              <tr key={row.no} className="border-t border-border/60">
      ...
      <div className="mt-3 flex items-center justify-between text-[11px] text-muted-foreground">
        <span>Menampilkan {per} dari {total}</span>
        <div className="flex items-center gap-1">
          <button onClick={() => setPage((p) => Math.max(1, p - 1))} className="rounded-md border border-border px-2 py-1 hover:bg-muted">‹</button>
          <span className="px-2 font-medium text-foreground tabular-nums">{page} / {pages}</span>
          <button onClick={() => setPage((p) => Math.min(pages, p + 1))} className="rounded-md border border-border px-2 py-1 hover:bg-muted">›</button>
        </div>
      </div>
    </Panel>
  );
  ...
  return (
    <Panel>
      ...
      <div className="mt-2 flex gap-3 text-[12px]">
        {BRANDS_TABS.map((t) => (
          <button key={t} onClick={() => setTab(t)} className={cn("font-medium", tab === t ? "text-foreground" : "text-muted-foreground")}>{t}</button>
        ))}
      </div>
...
function KontrolCard() {
  ...
  const tabs = ["Fraud", "Complain", "Kebersihan", "Event"];
  const rows = [
      ...
      <div className="mt-3 space-y-3">
        {rows.map((row) => (
          <div key={row.l}>
            ...
            <div className="mt-1"><Bar2 value={row.p * 4} /></div>
...
function AktivitasTerkini() {
  const [tab, setTab] = React.useState<"outlet" | "divisi">("divisi");
