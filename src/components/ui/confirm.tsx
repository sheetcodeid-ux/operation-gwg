...
type Pending = ConfirmOptions & { resolve: (v: string | boolean | null) => void };
...
export function useConfirm() {
  ...
  const open = React.useCallback((opts: ConfirmOptions) => {
    ...
    return new Promise<string | boolean | null>((resolve) => setPending({ ...opts, resolve }));
  }, []);

  /** true bila user menekan tombol konfirmasi, false bila membatalkan. */
  const confirm = React.useCallback(async (opts: ConfirmOptions) => (await open(opts)) !== null, [open]);
  ...
  const dialog = (
    <Dialog open={pending !== null} onOpenChange={(v) => !v && close(null)}>
      <DialogContent align="center" title={pending?.title ?? ""} description={pending?.description} className="max-w-sm">
        {pending?.prompt && (
