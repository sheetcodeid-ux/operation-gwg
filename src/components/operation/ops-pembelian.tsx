...
export function OpsPembelian({ month, rows: initial }: { month: string; rows: PurchaseRow[] }) {
  const router = useRouter();
  const pathname = usePathname();
  const [rows, setRows] = React.useState<PurchaseRow[]>(initial);
  ...
  function setCell(code: string, col: "warehouse" | "nonWarehouse", v: number) {
  ...
  function save() {
    start(async () => {
      const res = await savePurchasesAction(month, rows);
  ...
  async function downloadTemplate() {
    ...
    const data = rows.map((r) => ({ Kode: r.outletCode, Outlet: r.outletName, Warehouse: r.warehouse, "Non Warehouse": r.nonWarehouse }));
    const ws = XLSX.utils.json_to_sheet(data);
    const wb = XLSX.utils.book_new();
    XLSX.utils.book_append_sheet(wb, ws, "Pembelian");
    XLSX.writeFile(wb, `pembelian-${month}.xlsx`);
  }
  ...
  const gWh = rows.reduce((a, r) => a + r.warehouse, 0);
  const gNon = rows.reduce((a, r) => a + r.nonWarehouse, 0);
  ...
  return (
    <div className="space-y-4">
        ...
        <table className="w-full min-w-[40rem] border-collapse text-sm">
          <thead>
            <tr className="bg-muted/60 text-xs text-muted-foreground">
              ...
              <th className="px-3 py-3 text-right font-medium">Non Warehouse</th>
            ...
            {filtered.map((r) => (
              <tr key={r.outletCode} className="border-t border-border/60 hover:bg-foreground/5">
                ...
                <td className="px-1 py-1">
                  <input type="number" value={r.warehouse || ""} onChange={(e) => setCell(r.outletCode, "warehouse", Number(e.target.value) || 0)} className="w-36 rounded-md border border-transparent bg-transparent px-2 py-1.5 text-right text-[13px] tabular-nums outline-none hover:border-border focus:border-primary focus:bg-muted/30" placeholder="0" />
                </td>
                <td className="px-1 py-1">
                  <input type="number" value={r.nonWarehouse || ""} onChange={(e) => setCell(r.outletCode, "nonWarehouse", Number(e.target.value) || 0)} className="w-36 rounded-md border border-transparent bg-transparent px-2 py-1.5 text-right text-[13px] tabular-nums outline-none hover:border-border focus:border-primary focus:bg-muted/30" placeholder="0" />
                </td>
                <td className="px-3 py-2 text-right text-[13px] font-semibold tabular-nums text-foreground">{rp(r.warehouse + r.nonWarehouse)}</td>
              </tr>
            ))}
          </tbody>
          <tfoot>
            <tr className="border-t-2 border-border bg-muted/40 text-[13px] font-semibold text-foreground">
              ...
              <td className="px-3 py-2.5 text-right tabular-nums">{rp(gWh)}</td>
              <td className="px-3 py-2.5 text-right tabular-nums">{rp(gNon)}</td>
              <td className="px-3 py-2.5 text-right tabular-nums">{rp(gWh + gNon)}</td>
            </tr>
          </tfoot>
        </table>
      </div>
      ...
      <p className={cn("text-[12px]", dirty ? "text-amber-600 dark:text-amber-400" : "text-muted-foreground")}>
        {dirty ? "Ada perubahan belum disimpan — klik Simpan." : "Warehouse & Non-Warehouse dipisah. Data ini mengisi kartu Pembelian & Rencana Pengeluaran di Dashboard."}
      </p>
    </div>
  );
}
